...
Page({
  ...
  save: function () {
    ...
    if (this.data.edit_id) {
      ...
      Habit.update(this.data.edit_id, data)
        .then((res) => {
  ...
  habitAmount: function () {
    ...
    if (app.globalData.habitAmount == -1) {
      this.toast.showLoading();
      Habit.count(app.globalData.userInfo._openid)
        .then((res) => {
  ...
  loadHabit: function () {
    this.toast.showLoading();
    Habit.detail(this.data.edit_id)
      .then((res) => {
        this.toast.hideLoading();
        this.setData({
          title: res.title,
          ...
          date:
            res.end_time == null
              ? null
              : app.util.formatTime(res.end_time, false),
