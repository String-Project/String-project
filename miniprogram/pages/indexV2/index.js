...
Page({
  ...
  loadData: function () {
    ...
    Habit.loadToday(
      ...
      this.data.partnerInfo == null ? null : this.data.partnerInfo._openid,
    )
      .then((habits) => {
        Record.today(
          app.globalData.userInfo._openid,
  ...
  loadPartner: function () {
    User.myPartner(app.globalData.userInfo._openid)
      .then((res) => {
        ...
        if (lastPartnerInfo == null && res != null) {
          ...
          lastPartnerInfo != null &&
          res != null &&
          lastPartnerInfo._openid != res._openid
        ) {
  ...
  finishHabit: function (e) {
    ...
    Record.finish(habitId)
      .then((res) => {
        ...
        app.globalData.habits[my_lists[index]._id].times =
          my_lists[index].times;
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          Record.cancel(_id, habitId, my_lists[index].times)
            .then((res) => {
              if (that.data.partner) {
                my_lists[index].finish = false;
                my_lists[index].finish_id = "";
                my_lists[index].times =
                  my_lists[index].times > 0 ? my_lists[index].times - 1 : 0;
              } else {
                my_lists[index].finish = false;
                my_lists[index].finish_id = "";
                my_lists[index].times =
  ...
  partnerHabit: function (e) {
    ...
    habit.lastday =
      habit.end_time == null
        ? "∞"
        : parseInt((habit.end_time.getTime() + day - time) / day);

    if (habit.finish) {
      wx.showModal({
        title: habit.title,
        content:
          "打卡时间: " + app.util.formatTime(habit.finish_time, true, false),
        cancelText: "好的",
        confirmText: "查看详情",
      ...
      wx.showModal({
        ...
        success: function (res) {
          if (res.confirm) {
            that.toast.showLoading();
            let more =
              habit.lastday >= 0 && habit.lastday != "∞"
                ? "还剩" + habit.lastday + "天！"
                : "已打卡" + habit.times + "次！";
            Push.push(
              that.data.partnerInfo._openid,
              habit.title,
              "醒醒，你该打卡啦",
              more,
            )
