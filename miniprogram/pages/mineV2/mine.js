...
Page({
    ...
    this.setData({
      ...
      partnerTime:
        app.globalData.partnerInfo == null
          ? 0
          : app.util.sinceToday(app.globalData.partnerInfo.bind_time),
    });
  },
  ...
  loadData: function () {
    ...
    Habit.loadAll(app.globalData.userInfo._openid, this.data.page)
      .then((res) => {
        ...
        res.forEach((value, _) => {
          ...
          value.lastday =
            value.end_time == null
              ? "∞"
              : parseInt((value.end_time.getTime() + day - time) / day);
          lists.push(value);
        });
